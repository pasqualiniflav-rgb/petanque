...
Version du 20 septembre 2026 · révision 4 (pause multi, réglages de partie, compteur replié)
...
Maquettes de référence, canvas *Pétanque — Perso & vue 3ᵉ personne* : `Portrait.dc.html` est publiée dans le dépôt sous le nom `maquette-jeu-3e-personne.html` et fait foi pour la **disposition** de l'écran de jeu — sa charte ivoire relève du lot 2 et ne s'applique pas avant ; `Figures.dc.html` est publiée dans le dépôt sous le nom `maquette-choix-figure.html` et fait foi pour la galerie. `Camera-Basse.dc.html`, `Main.dc.html`, `Travelling.dc.html` et `BordTerrain.dc.html` sont en paysage 1280×720 : ce sont des recherches de cadrage, à lire comme intention et jamais comme gabarit.
...
## Lot 0 — Retouches sur l'écran existant

Trois corrections et une décision, à passer avant le lot 1 : elles touchent l'écran actuel et n'attendent pas le nouveau rendu.

### Le bandeau noir « Replay du dernier coup »

Dernier survivant du composant toast noir hérité, déjà chassé une fois. Il doit disparaître du parcours. Preuve par grep sur la chaîne de caractères **et** sur le composant, pas seulement sur le texte affiché. Si un message est nécessaire à ce moment du jeu, il passe par le composant de la charte en vigueur.

### Le compteur rétrécit et descend

Réduire encore la plaque et la descendre légèrement, pour que le ciel, les maisons et les platanes apparaissent au-dessus et derrière elle. La plaque cesse d'être collée au bord haut : elle se pose devant le décor au lieu de le masquer.

Contrainte à ne pas perdre : les deux rails de 1 à 13 restent lisibles et les jetons restent distinguables l'un de l'autre. Si la réduction demandée rend les crans illisibles, s'arrêter et le signaler avec une mesure, plutôt que rogner la lisibilité en silence.

### Pause et reprise

Deux fonctions différentes, à ne pas confondre :

|  | Partie contre l'ordinateur | Partie à plusieurs |
| --- | --- | --- |
| **Pause** | Vraie pause : le temps s'arrête, le chrono se fige | Partie privée : pause partagée. Partie classée : aucune pause |
| **Reprise** | Retrouver la partie en cours au retour | Rejoindre la partie qu'on avait quittée, tant qu'elle n'est pas finie |

Ce qui est demandé, en pratique :

- **En solo**, un vrai bouton pause qui gèle le chrono et la partie, et un bouton reprendre.
- **En partie privée** (des humains qui se sont partagé le lien), les deux existent : une vraie pause partagée ET le remplacement par un bot. Seul le joueur dont c'est le tour peut déclencher la pause — on ne peut donc pas geler la partie pendant le tour d'un autre, ce qui ôte tout intérêt à l'abus. La pause expire automatiquement au bout de quelques minutes, et le joueur absent est alors remplacé par un bot ; il récupère sa place à son retour, comme pour une déconnexion. En partie classée, pas de pause du tout.
- **Pause et reprise n'existent QUE dans l'app.** La version navigateur reste une version invité simplifiée : ni pause, ni reprise. Aucun stockage navigateur n'est donc à prévoir, la reprise étant adossée au compte.

Conséquence technique à ne pas rater : une pause en partie privée **ne peut pas rester locale**. Geler sa propre horloge ne gèle que son écran ; chez les autres le décompte continue et un appareil lance à la place de l'absent. Une pause partagée doit donc entrer dans l'état de partie et être honorée par tous les clients — c'est un changement par rapport à la conception validée pour le solo, où la pause restait volontairement locale.

## Lot 0 bis — Réglages de partie et finitions

### Points pour gagner

13 par défaut, avec 5 et 9 également proposés.

### Écran de création : valeurs par défaut

| Réglage | Valeur par défaut |
| --- | --- |
| Boules par joueur | 3 |
| Points pour gagner | 13 |
| Niveau des bots | Pointeur |
| Tournée de pastis | Sans |
| Cris du sud | Sans |
| Terrain | Classique |

Le panneau de réglages s'ouvre et se ferme, et il est **fermé à l'arrivée sur la page**. Ces valeurs par défaut doivent donc permettre de lancer une partie sans jamais l'ouvrir.

### « Revoir le coup » à l'écran de fin de partie

Le bouton ne fonctionne pas. Deux issues acceptables : il marche, ou il est grisé — même règle que dans la barre d'icônes. Un bouton présent et inerte est un bug visible.

### Le rectangle blanc des terrains 10 m

À identifier avant d'y toucher. Hypothèse de départ : la mini-carte du terrain long, rendue sans son contenu. Si c'est bien elle, la réparer (cadre de vue et position du cochonnet lisibles) plutôt que la supprimer : sur un terrain plus long que l'écran, elle dit où regarde la caméra. Si c'est autre chose, le signaler avant de décider.

### Le compteur : replié en jeu, complet au score

Cinquième demande de réduction. Le problème n'est plus la taille mais la forme, donc on change de forme plutôt que de rogner trois pixels de plus. Maquette de référence : `maquette-compteur.html`.

**Pendant la mène** — un bandeau replié : 28 px à deux équipes, 44 px à trois. Chaque équipe y garde son jeton posé sur son score, ses boules restantes et son verre de tournées. À deux équipes, les bleus sont collés à gauche et les rouges collés à droite, comme sur le terrain et comme sur la plaque, la ligne d'état au centre. À trois équipes il n'y a plus de gauche ni de droite : les trois groupes se répartissent dans l'ordre fixe des équipes et la ligne d'état passe dessous.

**Quand le score change** — la plaque complète s'ouvre deux secondes à la fin de la mène, le jeton grimpe jusqu'à son nouveau cran, puis elle se referme.

**Au clic sur le bandeau** — la même plaque s'ouvre et reste ouverte jusqu'à un nouveau clic sur le bandeau, ou un clic ailleurs. Pas de fermeture automatique quand c'est le joueur qui a ouvert : il regarde aussi longtemps qu'il veut.

Deux contraintes d'implémentation :

- La plaque s'affiche **en surimpression**, jamais en poussant le terrain vers le bas. Un terrain qui se décale de 40 px pendant que quelqu'un vise déplace sa cible sous son doigt.
- **Un seul composant** pour les deux déclencheurs, pas deux panneaux à maintenir en parallèle.

Gain mesuré sur maquette : 40 px de terrain à deux équipes, 60 px à trois — là où les quatre réductions précédentes en ont rendu une dizaine au total.

Deux points à vérifier plutôt qu'à supposer : la couleur de la troisième équipe dans le jeu (la maquette met un vert au jugé) et l'ordre d'affichage des équipes, qui doit reprendre celui que le jeu utilise déjà s'il en a un.

À vérifier sur téléphone réel : à trois équipes, trois groupes jeton + boules + verre sur 390 px de large, c'est dense. Si c'est trop chargé, retirer le verre du bandeau avant de toucher aux boules restantes — les tournées se lisent très bien dans la plaque ouverte, les boules servent pendant qu'on vise.

### Preuves attendues pour le lot 0

| Ce qui doit être prouvé | Preuve |
| --- | --- |
| Le bandeau noir a disparu | Grep sur la chaîne et sur le composant, les deux à zéro |
| Le compteur a rétréci sans perdre ses crans | Hauteurs avant/après et largeur par cran, mesurées |
| Le bandeau replié tient à deux et à trois équipes | Captures 390 px et 360 px, dans les deux configurations |
| La plaque ne pousse pas le terrain | Position du cercle de lancer identique, plaque ouverte et fermée |
| La pause ne s'affiche jamais en partie classée | Test dans les deux modes |
| La pause expire et bascule sur un bot | Test avec expiration forcée |
| Les réglages sont fermés à l'arrivée et les défauts corrects | Capture de la page de création au chargement |
| « Revoir le coup » marche ou est grisé | Capture de l'écran de fin |
